...
BATCH_SIZE = 4
miou_list = [0]
test_data = DataLoader(data_augmentation.Cam_test, batch_size=BATCH_SIZE, shuffle=True, num_workers=4)
net = FCN(32)
net.eval()
net = net.to(device)
...
for i, sample in enumerate(test_data):
    data = Variable(sample['img'].to(device))  # derivation?
    label = Variable(sample['label'].long().to(device))
    out = net(data)
    ...
    if len(eval_metrix['class_accuracy']) < 32:
...
epoch_str = ('test_acc :{:.5f}, test_miou:{:.5f}, test_mpa:{:.5f}, test_class_acc:{:}'.format(train_acc / (len(test_data) - error),
             train_miou / (len(test_data) - error), train_mpa / (len(test_data) - error), train_class_acc / (len(test_data) - error)))
