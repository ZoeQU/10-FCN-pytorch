...
for epoch in range(500):
    ...
    for i, sample in enumerate(train_data):
        imgdata = Variable(sample['img'].to(device))   # derivation?
    ...
    for j, sample in enumerate(val_data):
        valImg = Variable(sample['img'].to(device))  # derivation?
        ...
        eval_metrix = eval_semantic_segmentation(pre_label, true_label)
        eval_acc = eval_metrix['mean_class_accuracy'] + eval_acc
        eval_miou = eval_metrix['miou'] + eval_miou
    ...
    epoch_str = ('|Epoch|:{}\n|Train Loss|:{:.5f}\n|Train Acc|:{:.5f}\n|Train Mean IoU|:{:.5f}\n|Train Class acc|:{:}\n'
                 '|Valid Loss|:{:.5f}\n|Valid Acc|:{:.5f}\n|Valid Mean IoU|:{:.5f}\n|Valid Class acc|:{:}'.format(
                  ...
                  train_class_acc / len(train_data), eval_loss / len(val_data), eval_acc / len(val_data),
    ...
    if (max(best) <= eval_miou / len(val_data)):
        ...
        t.save(net.state_dict(), 'xxx.pth')  # tate_dict就是一个简单的Python dictionary，其功能是将每层与层的参数张量之间一一映射